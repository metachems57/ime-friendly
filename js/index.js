...
function renderHomepagePreviews() {
    ...
    if (reseauPosts.length > 0) {
        ...
        const reseauTitle = truncateText(lastReseauPost.title || lastReseauPost.content, 55);
        updateMediaPreview(
            'reseauPreviewImg',
            'reseauPreviewTitle',
            lastReseauPost.image || '',
            `Dernier post : "${reseauTitle || 'Sans titre'}"`,
            'Dernier post : Aucun post récent.'
        );
    }
    ...
    if (tools.length > 0) {
        ...
        const toolTitle = truncateText(lastTool.name, 55);
        const toolImage = lastTool.steps?.[0]?.images?.[0] || '';
        updateMediaPreview(
            'outilsPreviewImg',
            'outilsPreviewTitle',
            toolImage,
            `Dernier post : "${toolTitle || 'Sans titre'}"`,
            'Dernier post : Aucun contenu récent.'
        );
    }
